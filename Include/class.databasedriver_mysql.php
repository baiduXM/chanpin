...
echo $db->insertArray('test', array('username'=>'necylus', 'email'=>'lushaobo@126.com'), true);
echo $db->delete('delete from test where userid=1');
echo $db->updateArray('test', array('username'=>'卢少博'), array('id'=>intval($id)));
echo $db->update('update test set a=1,b=1 where id=1');
...
class DatabaseDriver_MySql {
	...
	private function init() {
		if(!$this->option['pconnect'])
			...
			$this->connect = mysql_pconnect($this->option['host'], $this->option['username'], $this->option['password']);
		if(!$this->connect){
			$this->halt();
		}
	...
	public function multi($recordcount, $page, $pagecount, $pagesize){
		...
		$data['limit_start'] = ($page-1)*$pagesize;
