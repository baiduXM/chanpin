 ...
 * @version 2.3 (2015.12.01 9:50 updated)
...
var PREVIEW_CONFIG = {
	...
	article: {
		...
		pubdate: '2014-12-25',
		pubtimestamp: 1419487200,
		viewcount: '<em id="article-viewcount">99</em>',
		category: {
			name: '导航A',
			en_name: 'Nav A',
			icon: '<i class="iconfont">&#xe60a;</i>',
...
var makeDataLogical = function() {
	...
	formatListsData('navs', 'data');
	formatListsData('navs', 'stardata');
	addStrongArticles(PREVIEW_CONFIG.list.data);
	!function() {
		...
		if (/\/(list-[a-z]+|searchresult)\.htm/.test(location.href)) {
			...
			var formatList = function(source) {
				...
				PREVIEW_CONFIG.list.icon = source.icon;
				...
				if (source.type == 4) {
					PREVIEW_CONFIG.list.content = source.content;
				}else if(source.type == 5){
					PREVIEW_CONFIG.list.content = source.content;
				}else{
...
function formatListsData (key, listkey) {
	if (typeof PREVIEW_CONFIG[key] !== 'object' || PREVIEW_CONFIG[key].length <= 0) return false;
...
function formatListData (value, listkey) {
	if (typeof value[listkey] !== 'object' || value[listkey].length <= 0) return false;
	switch (value.type) {
		case 2:
		case 3:
			for (var j=0; j<value[listkey].length; j++) {
				...
				value[listkey][j].category.icon = value.icon;
...
function addStrongArticles (listdata) {
	listdata[0].title = '<strong style="font-weight:normal;color:red;">' + listdata[0].title + '</strong>';
	listdata[1].title = '<strong style="font-weight:normal;font-style:italic;">' + listdata[1].title + '</strong>';
	listdata[2].title = '<strong style="font-weight:normal;text-shadow:1px 1px 3px;">' + listdata[2].title + '</strong>';
	listdata[3].title = '<strong style="font-weight:bold;">' + listdata[3].title + '</strong>';
}
		...
		var processResponse = function() {
			if (XMLHttpReq.readyState == 4) {
				if (XMLHttpReq.status == 200 || XMLHttpReq.status == 0) {
					if (!!XMLHttpReq.responseURL || !!XMLHttpReq.responseText) {
...
function saveConfig(index, data) {
	...
	for (var k in data) {
		...
		if (!checkDataK || typeof data[k].value === 'undefined' || typeof data[k].type === 'undefined' || typeof data[k].description === 'undefined')
			throw new Error(index + '.json文件中【'+k+'】变量不规范！\r\n每个变量的元素必须包含【value、type、description】元素，可选【config】\r\n详情参见：http://pme.eexx.me/doku.php?id=ued:template:json');
		// PHP端数据填充与验证
		slimming[k] = getFilledJsonData(data[k]);
	}
	function getFilledJsonData(dataK) {
		var slimmingK;
		if (typeof dataK.value !== 'undefined') {
			slimmingK = dataK.value;
		}else{
			slimmingK = dataK;
		}
		switch (dataK.type) {
			case 'text':
				if (typeof slimmingK !== 'string') throw new Error(index + '.json文件中【'+k+'】变量不规范！\r\ntype为【text】格式的value值必须为【字符串】\r\n详情参见：http://pme.eexx.me/doku.php?id=ued:template:json#typetext');
				break;
			case 'textarea':
				if (typeof slimmingK !== 'string') throw new Error(index + '.json文件中【'+k+'】变量不规范！\r\ntype为【textarea】格式的value值必须为【字符串】\r\n详情参见：http://pme.eexx.me/doku.php?id=ued:template:json#typetextarea');
				slimmingK = slimmingK.replace(/\r\n/g, "<br />").replace(/\n/g, "<br />");
				break;
			case 'image':
				if (typeof slimmingK.title === 'undefined' || typeof slimmingK.image === 'undefined' || typeof slimmingK.link === 'undefined')
					throw new Error(index + '.json文件中【'+k+'】变量不规范！\r\ntype为【image】格式的value值必须包含【title、image、link】元素，可选【description】\r\n详情参见：http://pme.eexx.me/doku.php?id=ued:template:json#typeimage');
				if (typeof dataK.config !== 'object' || (typeof dataK.config.width !== 'number' && typeof dataK.config.height !== 'number')) {
					var msg = index + '.json文件中【'+k+'】变量不规范！\r\ntype为【image】格式应配置【config】的【width】【height】配置项\r\n详情参见：http://pme.eexx.me/doku.php?id=ued:template:json#typeimage';
					if (typeof dataK.config === 'object' && dataK.config.forcesize === true) {
						throw new Error(msg);
					}else{
				...
				break;
			case 'images':
				if (typeof slimmingK !== 'object' || typeof slimmingK.length === 'undefined') throw new Error(index + '.json文件中【'+k+'】变量不规范！\r\ntype为【images】格式不正确！\r\n详情参见：http://pme.eexx.me/doku.php?id=ued:template:json#typeimages');
				for (var i=0; i<slimmingK.length; i++) {
					if (typeof slimmingK[i].title === 'undefined' || typeof slimmingK[i].image === 'undefined' || typeof slimmingK[i].link === 'undefined')
						throw new Error(index + '.json文件中【'+k+'】变量不规范！\r\ntype为【images】格式value值的每个子元素必须包含【title、image、link】元素，可选【description】\r\n详情参见：http://pme.eexx.me/doku.php?id=ued:template:json#typeimages');
				}
				if (typeof dataK.config !== 'object' || (typeof dataK.config.width !== 'number' && typeof dataK.config.height !== 'number')) {
					var msg = index + '.json文件中【'+k+'】变量不规范！\r\ntype为【images】格式应配置【config】的【width】【height】配置项\r\n详情参见：http://pme.eexx.me/doku.php?id=ued:template:json#typeimages';
					if (typeof dataK.config === 'object' && dataK.config.forcesize === true) {
						throw new Error(msg);
					}else{
						console.error('Error: ' + msg);
				...
				break;
			case 'page':
				dataK.config = typeof dataK.config === 'object' ? dataK.config : {};
				dataK.config.filter = typeof dataK.config.filter !== 'undefined' ? dataK.config.filter : "page";
			case 'list':
				dataK.config = typeof dataK.config === 'object' ? dataK.config : {};
				dataK.config.filter = typeof dataK.config.filter !== 'undefined' ? dataK.config.filter : "list";
			case 'link':
				dataK.config = typeof dataK.config === 'object' ? dataK.config : {};
				dataK.config.filter = typeof dataK.config.filter !== 'undefined' ? dataK.config.filter : "link";
			case 'nav':
				// if (TEMPLATE_CONFIG.Config.type == 2) throw new Error('手机模板上暂不支持【nav】（list、page、link）变量！\r\n请使用$mIndexCats，详情参见：http://pme.eexx.me/doku.php?id=ued:template:mindex#mindexcats_%E7%A7%BB%E5%8A%A8%E7%AB%AF%E9%A6%96%E9%A1%B5%E6%A0%8F%E7%9B%AE%E5%B1%95%E7%A4%BA%E6%95%B0%E6%8D%AE');
				dataK.config = typeof dataK.config === 'object' ? dataK.config : {};
				dataK.config.filter = typeof dataK.config.filter !== 'undefined' ? dataK.config.filter : "ALL";
				slimmingK = {
					name: '栏目B',
					en_name: 'Nav B',
					icon: '<i class="iconfont">&#xe60a;</i>',
				...
				if (typeof dataK.config === 'object' && dataK.config.filter == 'page') {
					// 仅获取单页类型的栏目
					slimmingK.content = '&lt;p&gt;文章内容...&lt;/p&gt;&lt;p&gt;一大段文章内容...&lt;/p&gt;';
					for (var i=typeof keyNavPage==='number'?keyNavPage:0; i<PREVIEW_CONFIG.navs.length; i++) {
						if (PREVIEW_CONFIG.navs[i].type == 4) {
							slimmingK.name = PREVIEW_CONFIG.navs[i].name;
							slimmingK.en_name = PREVIEW_CONFIG.navs[i].en_name;
							slimmingK.link = PREVIEW_CONFIG.navs[i].link;
							slimmingK.icon = PREVIEW_CONFIG.navs[i].icon;
							slimmingK.image = PREVIEW_CONFIG.navs[i].image;
							slimmingK.description = PREVIEW_CONFIG.navs[i].description;
							keyNavPage = i+1;
							break;
						}
					}
				} else if (typeof dataK.config === 'object' && dataK.config.filter == 'list') {
					// 仅获取列表类型的栏目
					slimmingK.data = objClone(PREVIEW_DEMODATA.list);
					for (var i=typeof keyNavList==='number'?keyNavList:0; i<PREVIEW_CONFIG.navs.length; i++) {
					...
					if (typeof dataK.config === 'object') {
						if (typeof dataK.config.limit === 'number') {
							slimmingK.data.splice(dataK.config.limit);
						}
						if (dataK.config.star_only === true) {
							for (var i=0; i<slimmingK.data.length; i++) {
								slimmingK.data[i].title = '★' + slimmingK.data[i].title;
							}
						}
					}
					if (typeof dataK.config !== 'object' || (typeof dataK.config.width !== 'number' && typeof dataK.config.height !== 'number')) {
						console.warn('Warn: '+index+'.json文件中【'+k+'】变量不规范！（做好图片自适应的列表或没调用图片展示 可忽略此警告）\r\ntype为【list】格式如果是图片或图文列表，应配置【config】的【width】【height】配置项\r\n详情参见：http://pme.eexx.me/doku.php?id=ued:template:json#typelist');
					}
					// 增加强调标题展示
					addStrongArticles(slimmingK.data);
				} else if (typeof dataK.config === 'object' && dataK.config.filter == 'link') {
					// 仅获取链接型的栏目
					for (var i=typeof keyNavLink==='number'?keyNavLink:0; i<PREVIEW_CONFIG.navs.length; i++) {
						if (PREVIEW_CONFIG.navs[i].type == 5) {
							slimmingK.name = PREVIEW_CONFIG.navs[i].name;
							slimmingK.en_name = PREVIEW_CONFIG.navs[i].en_name;
							slimmingK.link = PREVIEW_CONFIG.navs[i].link;
							slimmingK.icon = PREVIEW_CONFIG.navs[i].icon;
				...
				} else if (typeof dataK.config === 'object' && dataK.config.filter == 'feedback') {
					// 仅获取留言板栏目
					slimmingK.name = '留言板';
					slimmingK.en_name = 'Feed Back';
					...
					// 获取所有类型的栏目
					for (var i=typeof keyNav==='number'?keyNav:0; i<PREVIEW_CONFIG.navs.length; i++) {
						slimmingK.name = PREVIEW_CONFIG.navs[i].name;
						slimmingK.en_name = PREVIEW_CONFIG.navs[i].en_name;
						slimmingK.link = PREVIEW_CONFIG.navs[i].link;
						slimmingK.icon = PREVIEW_CONFIG.navs[i].icon;
				...
				// 获取子栏目文章
				if (typeof dataK.config === 'object' && dataK.config.showchilddata) {
					for (var i = 0; i < slimmingK.childmenu.length; i++) {
						slimmingK.childmenu[i].data = objClone(PREVIEW_DEMODATA.list);
					}
				}
				break;
			case 'navs':
				dataK.config = typeof dataK.config === 'object' ? dataK.config : {};
				slimmingK = [];
				testNavnum = typeof dataK.config.nav_limit === 'number' ? dataK.config.nav_limit : 6;
				for (var i = 0; i < testNavnum; i++) {
					slimmingK[i] = getFilledJsonData({
						type: 'nav',
						config: dataK.config
					});
				};
				break;
			case 'quickbar':
				if (TEMPLATE_CONFIG.Config.QuickBar != 'custom') console.warn('模板配置config.ini的Quickbar不是custom的话，json文件type为【quickbar】的变量无效！\r\n详情参见：http://pme.eexx.me/doku.php?id=ued:template:config&s[]=QuickBar#config_%E6%A8%A1%E6%9D%BF%E9%85%8D%E7%BD%AE%E9%83%A8%E5%88%86');
				if (typeof slimmingK !== 'object' || typeof slimmingK.length === 'undefined') throw new Error(index + '.json文件中【'+k+'】变量不规范！\r\ntype为【quickbar】格式不正确！\r\n详情参见：http://pme.eexx.me/doku.php?id=ued:template:mindex#%E5%BA%95%E9%83%A8%E5%AF%BC%E8%88%AA%E5%AE%9A%E4%B9%89%E6%96%B9%E6%B3%95');
				for (var i=0; i<slimmingK.length; i++) {
					if (typeof slimmingK[i].name === 'undefined' ||
						typeof slimmingK[i].image === 'undefined' ||
						typeof slimmingK[i].data === 'undefined' ||
						typeof slimmingK[i].type === 'undefined' ||
						typeof slimmingK[i].enable === 'undefined'
					) throw new Error(index + '.json文件中【'+k+'】变量不规范！\r\ntype为【quickbar】格式value值的每个子元素必须包【name、image、data、type、enable】元素，可选【childmenu】\r\n详情参见：http://pme.eexx.me/doku.php?id=ued:template:mindex#%E5%BA%95%E9%83%A8%E5%AF%BC%E8%88%AA%E5%AE%9A%E4%B9%89%E6%96%B9%E6%B3%95');
					if (!/^(tel|sms|im|link|share)$/.test(slimmingK[i].type))
						throw new Error(index + '.json文件中【'+k+'】变量不规范！\r\ntype为【quickbar】格式value值的子元素的【type】值只能为【tel、sms、im、link、share】其中之一\r\n详情参见：http://pme.eexx.me/doku.php?id=ued:template:mindex#%E5%BA%95%E9%83%A8%E5%AF%BC%E8%88%AA%E5%AE%9A%E4%B9%89%E6%96%B9%E6%B3%95');
					if (!slimmingK[i].enable) {
						slimmingK.splice(i, 1);
					}
					switch (slimmingK[i].type) {
						case 'tel':
							slimmingK[i].link = 'tel://' + slimmingK[i].data;
							break;
						case 'sms':
							slimmingK[i].link = 'sms://' + slimmingK[i].data;
							break;
						case 'share':
						case 'im':
							slimmingK[i].link = 'javascript:void(0);';
							break;
						case 'link':
							if (slimmingK[i].childmenu && slimmingK[i].childmenu.length>0) {
								slimmingK[i].link = 'javascript:void(0);';
								for (var j=0; j<slimmingK[i].childmenu.length; j++) {
									if (!slimmingK[i].childmenu[j].enable) {
										slimmingK[i].childmenu.splice(j, 1);
									}
									slimmingK[i].childmenu[j].link = slimmingK[i].childmenu[j].data;
								}
							}else{
								slimmingK[i].link = slimmingK[i].data;
							}
							break;
						default:
							slimmingK[i].link = slimmingK[i].data;
	...
	if (typeof applyCategoryDatabase === 'funtion') applyCategoryDatabase(index.replace('-','_'));
	return slimming;
}
...
onReady(function() {
	...
	AJAX('config.ini', function(config) {
		...
		// Change PREVIEW_CONFIG
		var enableNavData = function(targetNav, dataKey, level, nowL) {
			dataKey = dataKey ? dataKey : 'data';
			nowL = nowL ? nowL : 1;
			if (!targetNav || typeof targetNav !== 'object') return false;
			for (var i = 0; i < targetNav.length; i++) {
				targetNav[i][dataKey] = objClone(PREVIEW_DEMODATA.list);
				if (nowL < level) enableNavData(targetNav[i].childmenu, dataKey, level, nowL+1);
			}
		};
		if (parseInt(TEMPLATE_CONFIG.Config.EnableNavsData) >= 0) {
			enableNavData(PREVIEW_CONFIG.navs, 'data', parseInt(TEMPLATE_CONFIG.Config.EnableNavsData));
		}
		if (parseInt(TEMPLATE_CONFIG.Config.EnableNavsStarData) >= 0) {
			enableNavData(PREVIEW_CONFIG.navs, 'stardata', parseInt(TEMPLATE_CONFIG.Config.EnableNavsStarData));
		}
		if (parseInt(TEMPLATE_CONFIG.Config.EnablePagenavsData) >= 0) {
			enableNavData(PREVIEW_CONFIG.pagenavs, 'data', parseInt(TEMPLATE_CONFIG.Config.EnablePagenavsData));
		}
		if (parseInt(TEMPLATE_CONFIG.Config.EnablePagenavsStarData) >= 0) {
			enableNavData(PREVIEW_CONFIG.pagenavs, 'stardata', parseInt(TEMPLATE_CONFIG.Config.EnablePagenavsStarData));
		}
		// QuickBar
		if (TEMPLATE_CONFIG.Config.QuickBar != 'custom') {
			...
			if (TEMPLATE_CONFIG.Config.type == 2) window.previewJSQuickbar.type = 'm1';
			else window.previewJSQuickbar.type = 'p1';
		...
		if (typeof TEMPLATE_CONFIG.Config.Category === 'string') {
			...
			if (sourceCategoryDatabase) {
				AJAX(sourceCategoryDatabase, function(categoryDatabase) {
					try { categoryDatabase = eval('(' + categoryDatabase + ')'); }
					catch (e) { console.error('【'+sourceCategoryDatabase+'】文件内容不符合JSON格式！行业虚拟数据库未成功启用！！\r\n详情参见：http://pme.eexx.me/doku.php?id=ued:template#%E5%A6%82%E4%BD%95%E4%BF%AE%E6%94%B9%E7%A4%BA%E4%BE%8B%E6%95%B0%E6%8D%AE'); }
					// Change Category Database
					window.applyCategoryDatabase = function(variable) {
						if (typeof PREVIEW_CONFIG[variable] === 'undefined') return false;
						if (typeof categoryDatabase[variable] === 'object') {
							// e.g. PREVIEW_CONFIG.nav
							for(key in categoryDatabase[variable]){
								if (typeof PREVIEW_CONFIG[variable][key] !== 'undefined') {
									if (categoryDatabase[variable].length > 0) {
										// e.g. PREVIEW_CONFIG.nav[0]
										for(k in categoryDatabase[variable][key]){
											PREVIEW_CONFIG[variable][key][k] = categoryDatabase[variable][key][k];
										}
										if (categoryDatabase[variable].length > 5) PREVIEW_CONFIG[variable].splice(categoryDatabase[variable].length);
									}else{
										// e.g. PREVIEW_CONFIG.title
										PREVIEW_CONFIG[variable][key] = categoryDatabase[variable][key];
									}
								}else if(categoryDatabase[variable].length > 0 && typeof key === 'number'){
									// e.g. PREVIEW_CONFIG.nav[99]
									PREVIEW_CONFIG[variable][key] = categoryDatabase[variable][key];
								}
							}
						}else if(typeof categoryDatabase[variable] !== 'undefined') {
							PREVIEW_CONFIG[variable] = categoryDatabase[variable];
						}
					};
					for(variable in categoryDatabase){
						applyCategoryDatabase(variable);
					}
		...
		// After Changing Category Database
		function callbackChangeCategory() {
			...
			AJAX('!database.json', function(databaseData) {
				...
				callbackLoadDatabase();
			}, callbackLoadDatabase);
			
			// After Loading !database.json
			function callbackLoadDatabase() {
						...
						if (typeof document.querySelectorAll !== 'undefined') {
							...
							for (var imgs = document.querySelectorAll("img[src]:not([src^='{$site_url}'])"), i = imgs.length; i--;) {
								if (!/^http[s]?:\/\//.test(imgs[i].getAttribute("src")) && !/^\{\$.*\}$/.test(imgs[i].getAttribute("src"))) {
									console.error('Error: 所有的img标签引用“本地”图片src都必须加入路径变量【{$site_url}】\r\n详情参见：http://pme.eexx.me/doku.php?id=ued:template#%E6%A8%A1%E6%9D%BFjs_css%E5%BC%95%E7%94%A8');
								}
							}
						}
